...
function disassemble(script) {
  ...
  function readUInt16() {
    const uint = bytes.readUInt16LE(i);
    i += 2;
    return uint;
  }
  function readUInt32() {
    const uint = bytes.readUInt32LE(i);
    i += 4;
    return uint;
  }
  function readInt16() {
    const int = bytes.readInt16LE(i);
    i += 2;
    return int;
  }
  function readInt32() {
    const int = bytes.readInt32LE(i);
    i += 4;
    return int;
  }
  const entries = [];
  ...
  while (i < (bytes.length - 1)) {
    ...
    if (opcode >= OpCode.PUSHBYTES1 && opcode <= OpCode.PUSHBYTES75) {
      ...
      // case OpCode.PUSH0:
        ...
        {
          const script_hash = readBytes(20);
