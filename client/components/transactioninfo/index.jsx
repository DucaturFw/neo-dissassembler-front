...
class TransactionsInfo extends React.Component {
    ...
    return (
      <Fragment>
        <Title>
          ...
          <h3>
            {scriptinfo.type} | {scriptinfo.txid}
          </h3>
        </Title>
        <BlockInfo>
          <Block>
            <h4>List of instructions </h4>
            <BlockInstructions>{disassemble.map(item => <p>{item.name}</p>)}</BlockInstructions>
          </Block>
          <Block>
            <TitleStatus>Status : SUCCESS</TitleStatus>
            <Scroll>
              <Table>
                <Table.Header>
                  <Table.Row>
  ...
  render() {
    ...
    return <Wrap>{this.info(scriptinfo, disassemble)}</Wrap>;
  }
}
